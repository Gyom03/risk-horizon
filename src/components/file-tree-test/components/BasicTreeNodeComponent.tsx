...
import {
  ...
  DIVIDER_TITLE,
} from "./BasicTreeNodeTitleComponent"
    ...
    >
      {!node.isRoot() && (
        <div
          ...
          onClick={() =>
            treeController.expansions.setExpandedNode(node, !expanded)
          }
        >
          ...
          {hasChildren && (
            ...
            >
              {expanded && hasChildren ? (
                ...
                <FaAngleRight style={{ opacity: !hasChildren ? 0 : 1 }} />
              )}
            </div>
          )}
          {!hasChildren && (
            ...
            >
              {showBullets && !isDivider && (
                <FaCircle style={{ fontSize: "30%" }} />
              )}
            </div>
          )}
          <BasicTreeNodeTitleComponent
      ...
      {((expanded && hasChildren) || node.isRoot()) && (
        <div
          className={classNames(
            "d-flex flex-column gap-1 mt-1 select-none",
            !node.isRoot() && "ms-4"
          )}
        >
          {entryElements.length > 0 && (
            <div className="d-flex flex-column gap-1">{entryElements}</div>
          )}
        </div>
      )}
    </div>
