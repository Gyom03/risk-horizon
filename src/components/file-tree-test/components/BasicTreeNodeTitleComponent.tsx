...
export const BasicTreeNodeTitleComponent = ({
  ...
  React.useEffect(() => {
    if (
      ...
      treeController.focus.focusedNode?.getData()[
        treeController.options.idPropertyName
      ]
    ) {
    ...
    if (titleChanged) {
      timeout = setTimeout(() => {
        treeController.mutations.updateNode(node, {
          [treeController.options.titlePropertyName]: enteredTitle,
        })
  ...
  if (editable) {
    ...
    titleComponent =
      enteredTitle === DIVIDER_TITLE ? (
        ...
        <div
          className={
            "p-1 w-100 hover:bg-slate-100 duration-100 !cursor-pointer rounded-md" +
            (isSelected ? " !bg-slate-300" : "")
          }
          onClick={() => {
            /*console.log(node.getData()[treeController.options.idPropertyName])*/
            treeController.focus.setFocusedNode(node)
          }}
          style={{ fontSize: "105%" }}
        >
